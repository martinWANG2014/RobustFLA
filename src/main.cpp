...
void EchoUsage(){
    std::cout
            ...
            << "root_solution \t\t\t An boolean, 1, get the root solution;0, get the optimal solution." << std::endl
...
int main(int argc, char *argv[]) {
    ...
    try {
        ...
        if (boost::lexical_cast<int>(argv[9]) == 0) {
            ...
            ApproximateFLAEstimation(&network, vdParameters, boost::lexical_cast<double>(argv[6]) / 100.0,
                                     boost::lexical_cast<double>(argv[8]) / 100.0, &dSumBenefits, &iMaxNbConflict,
                                     boost::lexical_cast<int>(argv[4]),
                                     boost::lexical_cast<int>(argv[7]), iRandomMode);
        }
